...
@section('title', 'Edit Article \ Syllaboost')
@section('main')
<div class="max-w-3xl flex flex-col gap-6">
    ...
    <div class="flex items-center gap-4 mb-2">
        ...
        <h1 class="text-3xl font-bold text-zinc-900">Edit Article</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-sm shadow-zinc-100">
        ...
        <form action="{{ route('admin.articles.update', $article) }}" method="POST" class="p-6 flex flex-col gap-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-col gap-2">
                ...
                <textarea name="content" rows="12" required 
                    class="w-full rounded-xl border-zinc-300 bg-white px-4 py-3 text-zinc-900 placeholder:text-zinc-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all shadow-sm font-mono text-sm leading-relaxed">{{ $article->content }}</textarea>
            </div>
            
            <div class="flex items-center gap-3 p-4 bg-zinc-50 rounded-xl border border-zinc-100">
                <div class="flex items-center h-5">
                    <input type="checkbox" name="is_published" id="is_published" value="1" {{ $article->is_published ? 'checked' : '' }} 
                        class="size-4 rounded border-zinc-300 text-indigo-600 focus:ring-indigo-600 focus:ring-offset-0">
                </div>
                <div class="flex flex-col">
                    <label for="is_published" class="text-sm font-semibold text-zinc-900 cursor-pointer">Published</label>
